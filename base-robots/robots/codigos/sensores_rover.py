...
def clasificar_color(signature, profiles, tolerance=.15, margin=.04):
    if signature is None or len(profiles) < 3:
        return None
    distances = sorted((sum((a - b) ** 2 for a, b in zip(signature, profile)) ** .5, name)
...
def validar_config(cfg):
    ...
    positivo(cfg["color"], "settle_seconds", .2)
    ...
    for name, profile in cfg["color"]["profiles"].items():
        if name not in ("red", "green", "blue") or len(profile) != 3:
...
class SensoresRover:
    ...
    def update(self, moving=False):
        ...
        try:
            ...
            if self.phase == 4:
                c = self.cfg["color"]
                self.raw = self.samples[:]
                self.signature = firma_color(self.raw, c["polarity"], c["min_signal"])
                self.color = clasificar_color(self.signature, c["profiles"], c["tolerance"], c["margin"])
                ...
                self.hw.pixel[0] = lights[self.phase]
                self.next_color = self.clock() + self.cfg["color"]["settle_seconds"]
        except Exception as exc:
    ...
    def snapshot(self):
        return {"v": 1, "enabled": True, "distance_mm": self.distance,
                ...
                "color_calibrated": all(k in self.cfg["color"]["profiles"] for k in ("red", "green", "blue")),
