...
def send_command(command):
    ...
    try:
        ...
        status_label.config(text=f"🔴 Desconectado: {e}")
...
def push_cube(event=None):
    """Empuje fuerte y recto mientras se mantenga presionado."""
...
btn_up.bind("<ButtonPress-1>", lambda e: forward())
btn_up.bind("<ButtonRelease-1>", stop_move)

btn_down.bind("<ButtonPress-1>", lambda e: backward())
btn_down.bind("<ButtonRelease-1>", stop_move)

btn_left.bind("<ButtonPress-1>", lambda e: left())
btn_left.bind("<ButtonRelease-1>", stop_move)

btn_right.bind("<ButtonPress-1>", lambda e: right())
...
# Empuje fuerte: solo mientras se mantiene presionada la barra espaciadora.
root.bind("<KeyPress-space>", push_cube)
root.bind("<KeyRelease-space>", stop_move)


repeat_command()
root.after(200, connect_robot)
